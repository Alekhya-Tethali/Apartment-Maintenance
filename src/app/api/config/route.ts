...
const SAFE_KEYS = ["due_date_day", "admin_whatsapp_number", "webapp_url", "security_name", "admin_name", "backup_email_address"];
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (data.backupEmail !== undefined) {
      await upsertConfig("backup_email_address", data.backupEmail);
    }

    if (data.backupEmailPassword !== undefined) {
      await upsertConfig("backup_email_password", data.backupEmailPassword);
    }
